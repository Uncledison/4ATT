...
- **빠른 검사 / 정밀 검사 선택** — 강점 10문항(5분) 또는 강점+약점 20문항(원본 5 Minute Personality Test 구조, 10분)
- **강제 선택형 문항** — 4장의 카드를 나와 가까운 순서대로 탭하여 4→1점 부여
...
- **카카오톡 결과 공유** — 가족 동물 결과 요약 + 재검사 버튼을 카톡으로 전송
...
배포 주소:
- 정식: https://fun.uncledison.com/4att/ (Fununcle 사이트의 `public/4att/`에 정적 번들로 탑재)
- 백업: https://uncledison.github.io/4ATT/ (이 저장소의 GitHub Pages)

> 정식 배포 갱신 절차: `npm run build` 후 `dist/`를 Fununcle 저장소의 `public/4att/`에 복사하여 커밋하면 Vercel이 자동 배포합니다. Vite `base`가 상대경로(`./`)라 두 주소 모두에서 자산이 정상 로드됩니다.
